...
from djangoBLOG.settings import (
    BASE_DIR,
    GCP_API_CONSOLE_APP_PATH,
    GCP_API_CONSOLE_CLIENT_SCRECT_FILENAME,
)
import tomllib
import os.path
import datetime
...
class Command(BaseCommand):
    ...
    def findTargetInDrive(self, searcher=None, file_id=None, target_title=None):
        ...
        if target_title is None or file_id is None or searcher is None:
            return None
    ...
    def uploadFileToDrive(
        ...
        if file_name is None or drive_folder_id is None or local_file_path is None:
            self.stdout.write(
                self.style.ERROR(
                    self.getLoggerMsgWithTime(
                        'Wrong parameter in uploading file, SyncProcess is stop.'
                    )
                )
            )
            return False
        ...
        if os.path.exists(local_file_path) is False:
            self.stdout.write(
                self.style.ERROR(
                    self.getLoggerMsgWithTime(
                        '{} is not found in path: {}'.format(file_name, local_file_path)
                    )
                )
            )
            return False
        ...
        if file_id is None:
            self.stdout.write(
                self.style.WARNING(
                    self.getLoggerMsgWithTime(
                        '{} is not exist in drive, then SyncProcess created and upload to driver.'.format(
                            file_name
                        )
                    )
                )
            )
            update_file = uploader.CreateFile(
                {'parents': [{'id': drive_folder_id}], 'title': file_name}
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(
                    self.getLoggerMsgWithTime(
                        'SyncProcess update file: {} in driver.'.format(file_name)
                    )
                )
            )
    ...
    def handle(self, *args: Any, **options: Any) -> str | None:
        ...
        gauth = GoogleAuth(
            settings_file=BASE_DIR
            / (GCP_API_CONSOLE_APP_PATH + GCP_API_CONSOLE_CLIENT_SCRECT_FILENAME)
        )
        ...
        for config_item in config['gd_dest']:
            ...
            if config_item['order'] == 0:
                ...
                config_item['id'] = self.findTargetInDrive(
                    drive, config['gd_dest'][idx - 1]['id'], config_item['name']
                )
        ...
        for config_item in config['sync_files']:
            ...
            config_item['id'] = self.findTargetInDrive(
                drive, deepest_folder_id, config_item['name']
            )
            ...
            self.uploadFileToDrive(
                ...
                config_item['name'],
                deepest_folder_id,
                BASE_DIR / config_item['path'],
            )

        self.stdout.write(
            self.style.SUCCESS(
                self.getLoggerMsgWithTime('Finish SyncProceess: Backup database and files.')
            )
        )
